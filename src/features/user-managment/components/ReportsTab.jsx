...
function ReportsTab({ user }) {
  ...
  if (reports.length === 0) {
    return (
      <EmptyState
        title={t('adminUserDetail.reports.emptyTitle')}
        description={t('adminUserDetail.reports.emptyDescription')}
      />
    );
  }
    ...
    <div className="space-y-4">
      {reports.map((report) => {
        ...
        return (
          ...
          >
            <div>
              <p className="font-bold capitalize">{report.category || t('adminUserDetail.reports.defaultReport')}</p>
