...
function VerficationTab() {
  ...
  const handleResolve = (resolution) => resolveDoc({ id: docId, status: resolution });
    ...
    <div className="col-span-12 space-y-6 lg:col-span-8 xl:col-span-9">
      ...
      <div className="border-primary/10 rounded-xl border bg-white p-6 dark:bg-zinc-900">
        ...
        <div className="relative flex justify-between">
          ...
          <div
            className={`absolute top-5 left-0 z-0 h-0.5 transition-all ${isApproved ? 'w-full bg-green-500' : isRejected ? 'w-1/2 bg-red-400' : 'w-1/3 bg-primary'
              }`}
          />
          {[
            { label: 'Submitted', done: true, icon: <Check size={16} className="text-white" /> },
            { label: 'Under Review', done: isApproved || isRejected, icon: <EyeOff size={16} className="text-white" /> },
            { label: isApproved ? 'Verified' : isRejected ? 'Rejected' : 'Decision', done: isApproved, icon: <Check size={16} className="text-white" /> },
          ].map((step, i) => (
            <div key={i} className="relative z-10 flex flex-col items-center gap-3">
              <div
                className={`flex h-10 w-10 items-center justify-center rounded-full shadow-lg transition-all ${step.done
                  ? isRejected && i === 2 ? 'bg-red-400' : 'bg-green-500'
              ...
              <span
                className={`text-xs font-bold transition-all ${step.done
                  ? isRejected && i === 2 ? 'text-red-500' : 'text-green-600'
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <p className="font-bold">
                {isApproved ? 'Documents Approved' : isRejected ? 'Documents Rejected' : 'Awaiting Review'}
              </p>
            </div>
