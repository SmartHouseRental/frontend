                            ...
                            {images.length > 0 ? (
                                <>
                                    <div className="h-80 rounded-2xl bg-cover bg-center border border-border overflow-hidden transition-all duration-500"
                                        style={{ backgroundImage: `url('${images[selectedImage]?.url || images[selectedImage]}')` }}>
                                    </div>
                        ...
                        <CardContent>
                            ...
                            <div className="flex flex-wrap gap-2 mt-4">
                                {(property.amenities || []).map((tag, index) => (
                                    <span key={index} className="rounded-full bg-primary/5 border border-primary/10 px-3 py-1 text-xs font-medium text-primary">{getLocalizedText(tag, preferredLanguage)}</span>
                                ))}
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {images.map((img, i) => (
                            <div key={i} className="group relative h-48 rounded-xl overflow-hidden cursor-pointer">
                                <img src={img.url || img} alt={`Property photo ${i + 1}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
